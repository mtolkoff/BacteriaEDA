...
#                count = (count + 1) % len(list(matplotlib.colors.CSS4_COLORS.items()))
    ...
    clusterArrayResults = np.array(object = [[0] * 500] * 500, ndmin = 2)
    for i in range(0, 500):
        for j in range(j + 1, 500):
            if dbScanResults.labels_[i] == dbScanResults.labels_[j]: 
            #and dbScanResults.labels_[i] != -1:
                clusterArrayResults[i][j] = 1
    return clusterArrayResults
...
for k in range(20):
    ...
    for i in range(NUMBER):
        ...
        count += 1
        if(clusters.any() == 0):
            clusters = getClusterMatrix(data.sample(500, random_state = mySeed))
        else:
            clusters += getClusterMatrix(data.sample(500, random_state = mySeed))
    ...
    for i in range(500):
        for j in range(i + 1, 500):
            if clusters[i][j] > .5:
                countGrouped += 1
                pctGrouped += clusters[i][j]
            else:
                countUngrouped += 1
                pctUngrouped += clusters[i][j]
